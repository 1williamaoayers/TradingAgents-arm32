...
>
> 🆕 **新增特性**：
> - 🔄 **新闻智能去重** - 日期敏感 + 保留最完整版本（避免丢失"年内累计回购45亿"等增量信息）
> - 📊 **数据源实时优先** - 东财→多源快讯→Serper→RSS→数据库（最新消息排前面）
> - ⏰ **自动定时同步** - 新闻每2小时自动更新，无需手动操作
> - 🚀 **后端自动启动** - 容器启动时自动运行FastAPI后端+定时调度器
> - 🛠️ **开箱即用** - 自动补充JWT_SECRET等必需配置，小白也能一键部署
