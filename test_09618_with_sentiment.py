...
"""
港股09618完整测试 - 包含情绪分析
测试新的Alpha Vantage + FinnHub情绪分析工具
"""
...
print("=" * 80)
print("🧪 港股09618完整测试（含情绪分析）")
print("=" * 80)
print(f"测试时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print(f"股票代码: 09618 (京东集团-SW)")
print(f"市场类型: 港股")
print(f"新增功能: Alpha Vantage + FinnHub 情绪分析")
print("=" * 80)
...
TEST_CONFIG = {
    ...
    'analysts': ['market', 'social', 'news', 'fundamentals'],  # 包含social
...
try:
    ...
    if results.get('success'):
        ...
        if 'sentiment_report' in state and state['sentiment_report']:
            ...
            print(f"   预览: {state['sentiment_report'][:200]}...")
        else:
            print(f"\n⚠️ 市场情绪报告未生成")
        
        # 保存结果
        result_file = f"test_result_09618_with_sentiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
